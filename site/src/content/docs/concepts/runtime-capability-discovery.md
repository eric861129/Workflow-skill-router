...
Routing reads the capability description, domains, and stages to select semantic intent. Availability is a separate activation gate: an unavailable capability remains the intended route when it is the correct fit, but activation stops and reports a typed fallback instead of silently substituting a different Skill.
