...
這一段示範 router 要避免的問題：不要把所有「看起來相關」的 skills 全部塞進 context，而是選出最小但足夠的工作組合。
...
```text
客戶 dashboard 登入後整頁空白，請重現問題並修好。
```

Bad route：

```text
Route: 前端 / 所有相關項目 > Dashboard 問題
Use SKILL: `frontend-design`, `ui-styling`, `vue-expert`, `playwright`, `qa-test-planner`, `devops-engineer`
Reason: 這些 skills 都和 web 工作有關。
```

Better route：

```text
Route: 前端 / Vue / UI > 登入後瀏覽器回歸
Use SKILL: `vue-expert`, `systematic-debugging`, `playwright`
Reason: `vue-expert` 處理 component 與 state 行為；`systematic-debugging` 判斷空白畫面來自資料、權限或 render；`playwright` 建立可重複的瀏覽器驗證。
```
...
```text
API 更名了一個 response field，前端現在出現 type errors。請同步合約並更新 client。
```

Bad route：

```text
Route: 後端 / 前端 / 測試 / 文件 > 合約變更
Use SKILL: `csharp-developer`, `vue-expert`, `database-schema-designer`, `code-documenter`, `qa-test-planner`
Reason: 這個需求碰到很多 stack。
```

Better route：

```text
Route: API / 合約生命週期 > Schema diff 與 client generation
Use SKILL: `openapi-contract-generation-skill`, `openapi-to-typescript`, `api-designer`, `qa-test-planner`
Reason: `openapi-contract-generation-skill` 負責 schema diff；`openapi-to-typescript` 更新 client types；`api-designer` 檢查相容性；`qa-test-planner` 補上回歸覆蓋。
```
...
- Agent routing / Router 維護 / Skill inventory 更新: Primary: `workflow-skill-router`; Supporting: `skill-creator`, `writing-clearly-and-concisely`
- Skill authoring / 新增 Codex skill / Instruction package: Primary: `skill-creator`; Supporting: `openai-docs`, `code-documenter`
- Plugin packaging / Capability bundle / 可安裝 extension: Primary: `plugin-creator`; Supporting: `skill-installer`, `openai-docs`
- 影像生成 / 一般 raster asset / Prompted visual: Primary: `imagegen`; Supporting: `imagegen-frontend-web`, `frontend-design`, `brand`
