...
一般安裝請使用不可變的 `v2.0.0-beta.2` snapshot：
...
`v2.0.0-beta.2` 的 bundled local R0 支援 Personal Routing Profile 的 validate、install、list 與 preview。Contributor checkout 可直接安裝套件內的 personal 範例並執行 preview：
