...
使用 `v2.0.2` 時，先從一條重複工作流程的 Personal Routing Profile rule 開始。新增下一條前，用代表性的小型、phased 與 Managed Goal 請求執行 validate 與 `profile preview`。專案政策放在 `.codex/workflow-skill-router.json`；跨專案偏好放 personal。
