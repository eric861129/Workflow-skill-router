...
正好包含 20 筆 slot binding。每次 run 使用獨立的 HMAC-SHA-256 secret，分別承諾
task identity、source identity、Profile identity／revision、metric population flags
與 record integrity。Secret、原始任務、來源路徑及完整 manifest 都不得公開。
...
`manual_envelope`；`no_explicit_skill` 至少 10 筆；`explicit_lock` 至少 4 筆；
`router_local_resume` 至少 10 筆。四個母體可以重疊。
