...
Contract `workflow-skill-router.behavior-routing@2.2.0` binds the current-Phase oracle separately from the stateful Phase-transition oracle and requires scoped consent support to be justified by the current Phase's concrete exit evidence. Multi-turn consent and transition cases are scored at every declared turn, so a correct final route cannot conceal an earlier contract failure. Historical reports, including revision `2.1.0`, retain their original case and instruction digests and are never rescored against a newer oracle. Before execution, the runner recomputes the canonical path-and-SHA-256 manifest and fails closed if the instruction package no longer matches its declared digest.
