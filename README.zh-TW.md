...
修正版 36-attempt beta Behavior smoke 仍待明確 quota 授權；先前已 superseded 的 run 不是公開證據。十二個 case、每個 arm 重複三次，仍是 GA gate。有效 run 出現前公開狀態為 `manual-required`；執行後仍須通過 attestation 才能離開 `review-required`。
