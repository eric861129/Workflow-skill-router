...
> 已準備的 prerelease candidate：`2.0.0-beta.5`（尚未發布）。目前已發布的 V2 prerelease 仍是 `2.0.0-beta.3`；遷移期間仍可從 immutable V1.3.1 復原。
...
目前 checkout 包含 conditional-local loop 與 Host Integration Kit 的 **prepared beta.5 candidate**；它 **not included in published beta.3**，且尚未發布。這個第一個 candidate-content commit 刻意保留既有的 `release_source_revision`：後續受信任的 metadata-only promotion commit 必須將它綁定到此 exact reviewed candidate SHA，`Release V2` 才能 dispatch。
...
| 可用性 | Tools | 意義 |
| --- | --- | --- |
| `local-ready` | `plan_work`、`propose_support_consent`、`transition_support_consent`、`get_router_status` | 四個 bundled Router-local 操作 |
| `verified-host-required` | `sync_runtime_context`、`get_next_work`、`validate_route`、`record_work_event`、`evaluate_gate` | 五個操作需要 verified Host authority |
| `configured-adapter-required` | `run_model_evaluation`、`compare_evaluations`、`export_router_artifact` | 三個操作需要已授權的 evaluation adapter |

### 已準備的 beta.5 candidate（`prepared-beta.5-candidate`）

下列 prepared candidate matrix **not included in published beta.3**，也不代表 beta.5 已發布。在 exact candidate SHA 完成必要 evidence、review 與受信任的 metadata-only source binding 前，`prepared-local-candidate` lifecycle 會阻擋發布。
...
prepared beta.5 candidate 的正確宣稱是 **4 always local-ready + 3 Router-owned conditional-local**，不是 `7/12 local-ready`。Conditional-local 只在已驗證的 Router-owned work graph 且沒有 Native Goal authority 時執行；`get_next_work` 回傳 `authority_mode=router-local` 與 `host_goal_mutated=false`，進度和 local gate 使用 `evidence_class=user-or-agent-reported-local` 與 `host_transition_authorized=false`。Local gate 通過只代表 advisory 本機檢查，不是 Skill activation、Native Goal completion、deployment approval 或 production permission。Explicit Skill Lock 與 consent 行為不受影響。

| Runtime 情況 | `get_next_work` | `record_work_event` | `evaluate_gate` |
| --- | --- | --- | --- |
| 有效 Router-owned graph | Router-local 結果 | Router-local 回報式進度 | Router-local advisory gate |
...
| 缺少 Router-owned graph | `router-owned-work-graph`；在本機建立或 replay | `router-owned-work-graph`；在本機建立或 replay | `router-owned-work-graph`；在本機建立或 replay |
| Router-owned graph 損毀 | 已清理的 `internal-error` | 已清理的 `internal-error` | 已清理的 `internal-error` |

缺少 graph 是本機 graph 初始化條件，不應捏造 Host fallback。Graph 損毀代表完整性邊界失敗，公開回應只透過 `internal-error` 提供 correlation，內部細節只留在 diagnostics。Native Goal 則依上表使用各工具自己的 verified Host capability。
...
Evaluation contract `2.3.0` 保留目前 Phase oracle 與有狀態的 Phase-transition 評分，並把每次 Behavior run 綁定到乾淨、完整的 source revision 與已驗證的 adapter entrypoint identity。Runner 會在每次 adapter invocation 前後重新驗證來源與 adapter，因此 source 改變、adapter 被複製、改名或替換時，都不能繼續既有 run。六案例 beta profile 維持 36 attempts／42 model turns；十三案例 full gate 在每個 arm 重複三次時是 78 attempts／96 model turns。歷史 `2.2.0` reports 保留原本的 case 與 instruction digests，不得用較新的契約重算。新的 `2.3.0` Behavior run 在授權執行前維持 `manual-required`，執行後則在可信 attestation 前維持 `review-required`。
