...
const entries = tests
  ? ["bundled-runtime", "python-discovery", "runtime-readiness", "state-path", "tool-metadata", "tool-output", "tool-surface"].map((name) => ({ in: path.join(root, "mcp", "test", `${name}.test.ts`), out: path.join(root, ".test-build", `${name}.test.mjs`) }))
