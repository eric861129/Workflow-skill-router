...
5. 未指定 SKILL 時採 `auto`：先依 `workspace profile > personal profile > built-in` 解析 Personal Routing Profile，再選擇最小必要支援，不為 Router 推薦項目另外詢問同意。使用者當次明確指定 SKILL 時，Explicit Skill Lock 高於所有 Profile 偏好。
...
9. 已指定 SKILL 時採 `explicit-locked`：新增推薦支援前必須取得 scoped consent。MCP 可用時先以 `propose_support_consent` 綁定目前 Phase 的 route、scope、revision 與 concrete support set，再把使用者回覆分類成 `approved`、`rejected` 或 `unclear`。只有前兩者可呼叫 `transition_support_consent`；`unclear` 維持 pending。最終 route 只能採用 MCP 回傳值，不可讓模型在第二輪重寫。SKILL-only 只能提供 advisory transition。
10. 執行前宣告預計使用的 SKILL，完成後列出實際使用的 SKILL 與任何差異原因。
11. Profile 只提供 intended route；Runtime Capability Discovery、host hard constraints 與 R2/R3 approval 仍是 activation gate，能力不可用時 fail closed。
